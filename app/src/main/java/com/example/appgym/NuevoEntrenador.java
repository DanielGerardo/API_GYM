...
public class NuevoEntrenador extends AppCompatActivity {
    ...
    private Object[]header={"Nombre","Telefono","FechaDeIngreso","UserName"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ContextCompat.checkSelfPermission(NuevoEntrenador.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(NuevoEntrenador.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(NuevoEntrenador.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.CAMERA}, 1000);
        }
        ...
        sp1.setAdapter(item);
        TableDynamic.tableLayout.setOnClickListener(new View.OnClickListener() {
            @Override
...
public ArrayList<Object[]>getClients() {
    ...
    try {
        ps = cn.prepareStatement("SELECT Nombre,Telefono,FechaDeIngreso,UserName FROM Entrenador WHERE Activo=1");
        rs = ps.executeQuery();
        rsmd = rs.getMetaData();
   ...
   public void Limpiar(View view){
    ...
    public void PModificarMouseClicked(View view) {
        Conexion cc = new Conexion();
        Connection cn = cc.Conexion();
        String sql = "";
       ...
       // byte[] imagen = img.getBytes();
        ...
        String pass = et8.getText().toString();

        sql = "UPDATE Entrenador SET Nombre=?, Apellidos=?, Direccion=?, Telefono=?, Tel_Adicional=?, Sexo=?, Edad=?, FechaDeIngreso=GETDATE(), Fotografia=?, UserName=?, Pass=?, activo=1 WHERE UserName=?";
 if(Tel.length() == 0){et5.setText("null");}
       if (nom.length() == 0 || ape.length() == 0 || dire.length() == 0 || Tel.length() > 10 || edad<=0 || user.length() == 0 || pass.length() == 0) {
    ...
    public void PEliminarMouseClicked(View view) {
        Conexion cc = new Conexion();
        Connection cn = cc.Conexion();
        String sql = "DELETE Entrenador WHERE UserName=?";
        String user = et7.getText().toString();
   ...
   public void PGuardarMouseClicked(View view) {
       Conexion cc = new Conexion();
       Connection cn = cc.Conexion();
       String sql = "";
       ...
       if(Tel.length() == 0){et5.setText("null");}
       if (nom.length() == 0 || ape.length() == 0 || dire.length() == 0 || Tel.length() == 0&&Tel.length() > 10 || et6.getText().length()<=0 || user.length() == 0 || pass.length() == 0) {
 ...
 //et1.setTextColor(Color.parseColor("#03A9F4"));
    ...
    public void PBuscarMouseClicked() {
        Conexion cc = new Conexion();
        Connection cn = cc.Conexion();
        PreparedStatement ps;
        ResultSet rs;
        ...
        if (et1.getText().length() >= 0 || et2.getText().length() >= 0 || et3.getText().length() >= 0 || et4.getText().length() >= 0|| et5.getText().length() >= 0 || et6.getText().length() >= 0|| et8.getText().length() >= 0) {
            ...
            if (user.length() == 0) {
                ...
                try {
                    ps = cn.prepareStatement("SELECT Nombre,Apellidos,Direccion,Telefono,Tel_Adicional,Sexo,Edad,Fotografia,UserName,Pass FROM Entrenador WHERE UserName=?");
                    ps.setString(1, user);
                    rs = ps.executeQuery();
                    rsmd = rs.getMetaData();
